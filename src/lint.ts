export default (
  body: string,
  title: string,
  failureCallback: (message: string | Error) => void,
  ...
  if (!titleRegex.test(title)) {
    failureCallback(
      "Invalid PR title. Check the CONTRIBUTING.md file for more information."
    );
  }
  ...
  if (body.split("\n").length < 3) {
    failureCallback(
      "Invalid PR body. Needs a description of the WHY behind the change."
    );
  }
  ...
  if (bodyLines[bodyLines.length - 1].trim() === "") {
    ...
    const jiraRegex = new RegExp(
      "^(Relates to|Closes) ((?<!([A-Z]{1,10})-?)[A-Z]+-\\d+)$"
    );
    if (!jiraRegex.test(bodyLines[bodyLines.length - 1])) {
      ...
      if (regexResult?.[2] === "XYZ-123") {
        failureCallback(
          "Invalid PR body: The template Jira reference (XYZ-123) must be replaced."
        );
      }
